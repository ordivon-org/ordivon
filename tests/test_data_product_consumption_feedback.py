...
def test_research_post_registration_binding_does_not_claim_generation():
    ...
    assert r['consumerBinding']=='PASS_POST_REGISTRATION_OPENLINEAGE_PRODUCT_AND_EXISTING_RESULT_TO_VERIFICATION'
...
def test_finance_post_registration_loop_is_bounded_and_no_effect():
    ...
    assert f['consumerBinding']=='PASS_POST_REGISTRATION_PRODUCT_TO_DECISION_OUTCOME_FEEDBACK_NO_EXTERNAL_EFFECT'
    ...
    assert a['feedback']['standing']=='PASS_SELECTED_FINANCE_EXPLICIT_NO_CHANGE_DISPOSITION'
...
def test_r5_has_no_selected_p0_but_remains_cross_domain_partial():
    ...
    assert by['feedback-recollection']['standing']=='PASS_SELECTED_FINANCE_NO_CHANGE_DISPOSITION_NONTRIVIAL_UPDATE_UNPROVEN'
    p1={x['id'] for x in r5['p1Queue']}
    assert {'nontrivial-feedback-update','business-value-outcome','broader-domain-data-product-adoption'} <= p1
