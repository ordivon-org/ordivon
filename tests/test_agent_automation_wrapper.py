...
import agent_automation_wrapper as w  # noqa: E402
...
class AgentAutomationWrapperTests(unittest.TestCase):
    def test_wrapper_targets_browserless_temporal_facade_not_legacy_tool(self):
        text = (ROOT / "scripts/agent_automation_wrapper.py").read_text()
    ...
    def test_wrapper_delegates_facade_to_dependency_bearing_control_runtime(self):
        ...
        with (
            ...
            patch.object(
                sys,
                "argv",
                ["agent-automation", "provider-preflight", "--endpoint-id", "carrier-a"],
            ),
        ):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            with (
                ...
                with self.assertRaisesRegex(SystemExit, "HOLD_CLOSED"):
                    with w._admission_read_lease("birth"):
                        pass
                with w._admission_read_lease(None):
                    pass
    ...
    def test_mutating_cli_lease_blocks_behind_cross_process_release_fence(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            code = (
                ...
                "import agent_automation_wrapper as w; r=Path(sys.argv[2]); "
            ...
            with lock.open("a+") as handle:
                ...
                time.sleep(0.1)
                self.assertIsNone(child.poll(), "mutating CLI crossed an exclusive release fence")
                fcntl.flock(handle.fileno(), fcntl.LOCK_UN)
                out, err = child.communicate(timeout=5)
            self.assertEqual(child.returncode, 0, err)
            self.assertEqual(out.strip(), "ADMITTED")

    def test_browserless_cli_birth_and_continue_are_control_plane_admissions(self):
        text = (ROOT / "scripts/agent_automation_browserless.py").read_text()
        self.assertIn('elif a.action == "birth":', text)
        self.assertIn('r = svc.launch_occurrence(a.spec, a.agent_id)', text)
        self.assertIn('elif a.action == "continue":', text)
        self.assertIn('r = svc.launch_continue(', text)
        self.assertNotIn('elif a.action=="birth": r=svc.birth(', text)
        self.assertNotIn('elif a.action=="continue": r=svc.continue_occurrence(', text)
