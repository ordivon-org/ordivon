...
def test_prov_keeps_research_derivation_and_records_finance_consumption():
    ...
    assert a['research']['consumerBinding']=='PASS_POST_REGISTRATION_OPENLINEAGE_PRODUCT_AND_EXISTING_RESULT_TO_VERIFICATION'
    assert a['research']['generationClaim']=='EXPLICITLY_NOT_CLAIMED'
    assert a['finance']['consumerBinding']=='PASS_POST_REGISTRATION_PRODUCT_TO_DECISION_OUTCOME_FEEDBACK_NO_EXTERNAL_EFFECT'
...
def test_acceptance_and_census_do_not_overclaim():
    ...
    assert 'post-registration consumption remains unproven' not in a['claimBoundary'].lower()
