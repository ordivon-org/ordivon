...
CATALOG = STUDY / "data/scholarly-data-catalog-r1.json"
PLAN = STUDY / "data/scholarly-data-acquisition-plan-r1.json"
SD1 = STUDY / "data/sd1-acquisition-readiness-r1.json"
SD2 = STUDY / "data/sd2-review-lifecycle-readiness-r1.json"
ARIES_RECEIPT = STUDY / "evidence/aries-bounded-core-r1.json"
CONTEXT24_TRANSPORT = STUDY / "evidence/context24-transport-blocker-r1.json"
ARIES_BASELINE = STUDY / "evidence/aries-review-revision-baseline-r1.json"
...
ARIES_RESPONSE_RECEIPT = STUDY / "evidence/aries-review-response-core-r1.json"
ARIES_SEMANTIC_RECEIPT = STUDY / "evidence/aries-semantic-content-core-r1.json"
ARIES_FORK_BASELINE = STUDY / "evidence/aries-lifecycle-fork-baseline-r1.json"
...
def verify_emse(asset: dict[str, Any]) -> dict[str, int]:
    ...
    if (
        grammar["cleanBaselineFulltextN"]
        != observed["accessibleRandomBaselineFulltexts"]
    ):
    ...
    if (
        count_csv_rows(resolved["acquisitionManifest"])
        != observed["validatedFulltexts"]
    ):
...
def verify_aries(asset: dict[str, Any], receipt: dict[str, Any]) -> dict[str, int]:
    ...
    receipt_hashes = {
        ...
        "NORMALIZATION_SUMMARY_R1.json": receipt["snapshot"][
            "normalizationSummarySha256"
        ],
        "ANALYTICAL_BUILD_RECEIPT_R1.json": receipt["snapshot"][
...
def verify_disapere(asset: dict[str, Any], receipt: dict[str, Any]) -> dict[str, int]:
    ...
    if (
        receipt.get("authority", {}).get("commercialProductOrServiceUse")
    ...
    required = [
        ...
        "NORMALIZATION_SUMMARY_R1.json",
        "ANALYTICAL_BUILD_RECEIPT_R1.json",
        "derived/jsonl/pairs.jsonl",
        "derived/jsonl/review_sentences.jsonl",
        "derived/jsonl/rebuttal_sentences.jsonl",
        "derived/jsonl/local_alignment_links.jsonl",
        "derived/parquet/pairs.parquet",
        "derived/parquet/review_sentences.parquet",
    ...
    if (
        sha256(root / "raw/DISAPERE-upstream.zip")
        != receipt["source"]["releaseZipSha256"]
    ):
    ...
    expected_census = {
        "pairFiles": 506,
        "reviewSentences": 9946,
        "rebuttalSentences": 11103,
        "localAlignmentLinks": 21675,
        "orphanLocalAlignmentLinks": 0,
        "duplicateReviewSentenceIdentities": 0,
    }
    for key, expected in expected_census.items():
        if census["counts"].get(key) != expected:
            fail(f"DISAPERE census drift: {key}")
    if "commercial product/service use is not authorized" not in census.get(
        "licenseBoundary", ""
    ):
    ...
    if analytical.get("counts") != {
        "pairs": 506,
        "review_sentences": 9946,
        "rebuttal_sentences": 11103,
        "local_alignment_links": 21675,
    }:
...
def main() -> int:
    ...
    if (
        ...
        != "MATERIALIZED_REVIEW_RESPONSE_CORE_ANALYTICAL_VIEWS_PASS"
    ):
        fail("ARIES response-core admission standing drifted")
    if (
        aries_response_receipt.get("parentAsset", {}).get("snapshotIdentity")
        != aries_receipt["snapshot"]["identity"]
    ):
        fail("ARIES response-core parent binding drifted")
    if (
        aries_fork_baseline.get("sourceSnapshotIdentity")
        != aries_response_receipt["snapshot"]["identity"]
    ):
        fail("ARIES lifecycle fork baseline identity drifted")
    if (
        aries_fork_baseline.get("counts", {}).get("manualConcernsWithBothBranches")
        != 87
    ):
    ...
    if (
        disapere_baseline.get("sourceSnapshotIdentity")
        != disapere_receipt["snapshot"]["identity"]
    ):
    ...
    if disapere_baseline.get("counts") != {
        ...
        "rebuttalSentences": 11103,
        "localAlignmentLinks": 21675,
        "locallyAlignedRebuttalSentences": 9416,
        "locallyTargetedReviewSentences": 4096,
        "requestLinkedRebuttalSentences": 5220,
    }:
    ...
    if (
        peersum_receipt.get("status")
        != "MATERIALIZED_DIGEST_BOUND_ANALYTICAL_VIEWS_PASS"
    ):
    ...
    if (
        peersum_baseline.get("sourceSnapshotIdentity")
        != peersum_receipt["snapshot"]["identity"]
    ):
        fail("PeerSum baseline source identity drifted")
    if (
        peersum_baseline.get("splitCountStanding")
        != "CURRENT_CARRIER_DIFFERS_FROM_HISTORICAL_DOCUMENTATION"
    ):
        fail("PeerSum split drift boundary missing")

    if (
        context24_receipt.get("status")
        != "MATERIALIZED_IDENTITY_CORE_ANALYTICAL_VIEWS_PASS"
    ):
    ...
    if (
        context24_baseline.get("sourceSnapshotIdentity")
        != context24_receipt["snapshot"]["identity"]
    ):
        fail("Context24 baseline source identity drifted")

    if (
        context24_content_receipt.get("status")
        != "MATERIALIZED_PARTIAL_GOLD_EVIDENCE_CONTENT_CORE_PASS"
    ):
        fail("Context24 content-core admission standing drifted")
    if (
        context24_content_receipt.get("parentAsset", {}).get("snapshotIdentity")
        != context24_receipt["snapshot"]["identity"]
    ):
        fail("Context24 content-core parent identity drifted")
    if (
        context24_content_baseline.get("sourceSnapshotIdentity")
        != context24_content_receipt["snapshot"]["identity"]
    ):
        fail("Context24 content baseline identity drifted")
    expected_context24_content = {
        "task1TrainRows": 474,
    ...
    if context24_content_receipt.get("counts") != expected_context24_content:
        fail("Context24 content-core counts drifted")
    rights = context24_content_receipt.get("rights", {})
    ...
    if rights.get("externalRedistribution") != "NOT_AUTHORIZED_BY_CATALOG":
        fail("Context24 external redistribution silently authorized")
    if rights.get("commercialMediaReuse") != "NOT_AUTHORIZED_BY_CATALOG":
        fail("Context24 commercial media reuse silently authorized")
    if (
        by_asset["context24-evidence-content-core-r1"].get("observedScale")
        != expected_context24_content
    ):
        fail("Context24 content catalog observedScale drifted")
    ...
    for row in candidates:
        ...
        if not standing or not any(
            token in standing for token in ("VERIFY", "MUST_BE_BOUND", "OBSERVED")
        ):
    ...
    if (
        by_candidate["context24"].get("acquisitionState")
        != "MATERIALIZED_IDENTITY_PLUS_PARTIAL_CONTENT_CORE"
    ):
        fail("Context24 candidate/local asset state mismatch")
    if (
        by_candidate["disapere"].get("acquisitionState")
        != "MATERIALIZED_BOUNDED_CORE_NONCOMMERCIAL"
    ):
    ...
    if (
        by_wave.get("SD1", {}).get("standing")
        != "PARTIAL_CONTEXT24_IDENTITY_AND_CONTENT_CORE_MATERIALIZED"
    ):
    ...
    if admission.get("mayMaterializeWhenTransportAvailable") != []:
        fail(
            "Context24 must no longer be transport-blocked after bounded materialization"
        )
    if admission.get("materialized") != [
        "context24-identity-core-r1",
        "context24-evidence-content-core-r1",
    ]:
    ...
    result = {
        ...
        "ariesLifecycleFork": {
            ...
            "withPositiveRevision": 87,
            "withBothBranches": 87,
        },
        "disapere": disapere,
        "peersum": peersum_receipt["counts"],
        "context24": context24_receipt["counts"],
        "context24Content": expected_context24_content,
        "context24ContentCoverage": {
            "contentBoundGoldLinks": 256,
            "identityOnlyGoldLinks": 423,
            "goldContentLinkFraction": 256 / 679,
            "task1TrainFulltext": "229/229",
            "task1TestFulltext": "46/46",
        },
        "context24HistoricalDirectTransport": context24_transport["standing"],
        "ariesManualBaseline": expected_baseline,
        "disapereCoverageBaseline": {
            key: {"targeted": value[0], "total": value[1]}
