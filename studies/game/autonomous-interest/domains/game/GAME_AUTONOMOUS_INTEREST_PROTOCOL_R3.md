# Autonomous Game Interest R3 — Declarative, Precommitted, No-Live-Human Search
...
R2 correctly terminated with `0/12` semantic slots consumed. R3 treats those findings as protocol-design evidence only.
...
After Human unblinding, the same R3 experiment cannot change candidate logic, balance, tutorial, pacing, art or thresholds and remain R3.

---

# 3. Freshness by provenance, not subjective similarity

R3 removes post-generation “looks too similar” rejection from the selection law because that would reintroduce evaluator discretion.

A fresh R3 candidate is instead defined by provenance:

- its authoritative model bytes are first authored **after** the exact R3 protocol freeze;
- exactly three fresh generator occurrences independently own `R3C01`, `R3C02`, `R3C03`;
- before fixing their model, generators may read the frozen R3 protocol, mechanism/motif libraries, public design priors and mature implementation patterns;
...
R3 does not use semantic similarity to eliminate a candidate after generation. Low-level mature primitive reuse remains allowed.

---

# 4. Search budget and phase structure

R3 intentionally removes the 12→6→3 preimplementation selection layer.
...
`domains/game/game-autonomous-interest-model-r3.schema.json`
...
R3 does not attempt to sample whether candidate logic is deterministic. The candidate model has no executable transition code, RNG callback, network access, wall clock, environment access, policy identity or candidate reward.
...
`experiments/autonomous-interest-r3/evaluator_reference.py`
...
All schema/semantic-valid R3 candidates are evaluated; there is no QD preselection.
...
For every winner coupling, retrain P1/P2 and reevaluate the complete surrogate suite **inside that ablated world**. An ablation is eligible only if P3 success remains `>=0.50`.
...
- measured complexity caps pass;
- reference evaluation completes;
- candidate logic contains no arbitrary executable code.

This is the complete automated game-logic gate for R3; evaluators may not add candidate-specific runtime-quality criteria after generation.

Presentation/renderer defects are handled separately before final Human freeze and cannot alter the selected model logic.

---

# 14. Human renderer boundary

Automated selection authority is the declarative model only.

After a winner is selected, a generic renderer may present that model to Humans. It must:

- use the same frozen transitions, observations, actions, statuses and coupling ablation semantics;
- contain no candidate-specific gameplay rule code;
- pass model↔renderer conformance replay before freeze;
- preserve the exact selected winner/ablation logic.

Art/presentation may instantiate generic visual slots, but cannot alter game state transitions or selection evidence.
...
Before any R3 generator occurrence is allowed to create `R3C01–R3C03`, the complete R3 artifacts must be committed and revalidated from a fresh main workspace.
...
Any protocol-level defect terminates/version R3. It may not be repaired in place after the frozen commit is exposed to candidate generation.
...
```text
R3_PROTOCOL_FROZEN               false   # becomes true only at exact freeze commit
R3_PREGEN_AUDIT_PASSED           false
R3_CANDIDATE_GENERATION_STARTED  false
R3_SEMANTIC_SLOTS_CONSUMED       0
R3_EXACT_FRESH_CANDIDATES        3
