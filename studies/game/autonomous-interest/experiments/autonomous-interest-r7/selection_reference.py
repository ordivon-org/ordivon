...
def tournament_select(models: Sequence[Mapping], *, allow_selftest: bool=False) -> dict:
    ...
    ordered_models=sorted(models,key=lambda m:m["candidateId"])
    evaluations=[evaluate_authoritatively(m,allow_selftest=allow_selftest) for m in ordered_models]
    ...
    model_by_id={m["candidateId"]:m for m in ordered_models}
    ...
    for sid in model["states"]:
        ...
        for action,tr in st["transitions"].items():
            ...
            records.append({
                ...
                "disabled":er.semantic_state_identity(model,tr["withoutCouplingNext"],coupling),
            })
    if not records:
