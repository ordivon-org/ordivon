...
HARD_CAPS = {
    ...
    "maxObservationJsonNodes": 64,
    "maxObservationBytes": 2048,
}
...
def validate_model(model: Mapping, allow_selftest: bool = False) -> dict:
    ...
    if model["kind"] == "candidate":
        if model["candidateId"] not in {"R3C01","R3C02","R3C03"}:
            raise ValueError("invalid candidate id")
    elif not (allow_selftest and model["kind"] == "selftest" and model["candidateId"] == "R3SELFTEST"):
    ...
    for sid, st in states.items():
        ...
        obs = st["observation"]
        if not isinstance(obs, dict) or len(obs) > 12:
            raise ValueError(f"invalid observation {sid}")
...
def initial_states(model: Mapping, split: str, context: int) -> List[str]:
    ...
    # Input array ordering is representation only, not policy-training authority.
    return sorted(_ctx(model, context)[key])
...
def run_episode(model: Mapping, start: str, policy: str, table=None, train=False, epoch=0, context=0, disabled_coupling=None) -> dict:
    ...
    for step in range(MAX_EPISODE_DECISIONS):
        ...
        if policy == "P0":
            ...
            if train and deterministic_float(model["candidateId"],policy,epoch,context,start,step,key) < epsilon(epoch):
                idx = int(deterministic_float("pick",model["candidateId"],policy,epoch,context,start,step,key) * len(legal)) % len(legal)
                action = legal[idx]
            else:
