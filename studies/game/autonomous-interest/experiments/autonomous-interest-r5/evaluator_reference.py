...
HARD_CAPS = {
    ...
    "uniqueObservationCount": 256,
    "maxObservationJsonNodes": 13,
...
def all_state_semantic_classes(model: Mapping, disabled_coupling: Optional[str] = None) -> Dict[str, int]:
    ...
    for sid in states:
        st=model["states"][sid]
        base[sid]=(st["status"],canonical_json(st["observation"]),tuple(ordered_legal(model,sid)))
    ...
    while True:
        ...
        if all(new[sid]==cls[sid] for sid in states):
            _SEMANTIC_CLASS_CACHE[key]=new
            return new
        cls=new

def semantic_state_identity(model: Mapping, state_id: str) -> str:
    cls=all_state_semantic_classes(model)
    return canonical_json({"visible":visible_key(model,state_id),"futureClass":cls[state_id]})
...
def validate_model(model: Mapping, allow_selftest: bool = False) -> dict:
    ...
    if model["kind"] == "candidate":
        if model["candidateId"] not in {"R5C01","R5C02","R5C03"}:
            raise ValueError("invalid candidate id")
    elif not (allow_selftest and model["kind"] == "selftest" and model["candidateId"] in {"R5SELFTEST","R5SELFTEST01","R5SELFTEST02","R5SELFTEST03"}):
    ...
    for sid, st in states.items():
        ...
        obs = st["observation"]
        if not isinstance(obs, list) or len(obs) != 12 or any(type(v) is not int or v < 0 or v > 15 for v in obs):
            raise ValueError(f"invalid neutral observation {sid}")
        if json_node_count(obs) > HARD_CAPS["maxObservationJsonNodes"]:
            raise ValueError(f"observation node cap {sid}")
        if len(canonical_json(obs).encode("utf-8")) > HARD_CAPS["maxObservationBytes"]:
            raise ValueError(f"observation byte cap {sid}")
    ...
    measured = {
        ...
        "uniqueObservationCount": len({canonical_json(states[s]["observation"]) for s in reachable}),
        "maxObservationJsonNodes": max(json_node_count(states[s]["observation"]) for s in reachable),
        "maxObservationBytes": max(len(canonical_json(states[s]["observation"]).encode("utf-8")) for s in reachable),
    }
...
def initial_states(model: Mapping, split: str, context: int) -> List[str]:
    ...
    return sorted(_ctx(model, context)[key], key=lambda sid: semantic_state_identity(model,sid))


def reachable_states(model: Mapping, disabled_coupling: Optional[str] = None) -> set[str]:
    starts = []
    for c in range(4):
        starts += initial_states(model, "train", c) + initial_states(model, "eval", c)
    seen = set(starts)
...
def bisimulation_classes(model: Mapping, disabled_coupling: Optional[str] = None) -> Dict[str, int]:
    ...
    for s in states:
        st = model["states"][s]
        base[s] = (st["status"], canonical_json(st["observation"]), tuple(ordered_legal(model, s)))
    uniq = {v:i for i,v in enumerate(sorted(set(base.values()), key=repr))}
...
def visible_key(model: Mapping, state_id: str) -> str:
    return canonical_json({"observation": model["states"][state_id]["observation"], "legalActions": ordered_legal(model, state_id)})
...
def run_episode(model: Mapping, start: str, policy: str, table=None, train=False, epoch=0, context=0, disabled_coupling=None) -> dict:
    ...
    for step in range(MAX_EPISODE_DECISIONS):
        ...
        if policy == "P0":
            ...
            if train and deterministic_float(model["candidateId"],policy,epoch,context,semantic_state_identity(model,start),step,key) < epsilon(epoch):
                idx = int(deterministic_float("pick",model["candidateId"],policy,epoch,context,semantic_state_identity(model,start),step,key) * len(legal)) % len(legal)
                action = legal[idx]
            else:
...
def train_q(model: Mapping, memory: bool, only_context: Optional[int] = None, disabled_coupling: Optional[str] = None):
    ...
    for epoch in range(TRAIN_EPOCHS):
        outcomes = []
        for c in contexts:
            for start in initial_states(model,"train",c):
...
def eval_policy(model: Mapping, policy: str, table=None, disabled_coupling=None) -> List[dict]:
    rows=[]
    for c in range(4):
        for start in initial_states(model,"eval",c):
...
def aliasing_rate(model: Mapping, disabled_coupling: Optional[str] = None) -> float:
    ...
    for s in sorted(reachable_states(model, disabled_coupling)):
        if model["states"][s]["status"]=="ONGOING":
            groups[canonical_json(model["states"][s]["observation"])].append(s)
    den=num=0
...
def evaluate_model(model: Mapping, allow_selftest: bool=False, disabled_coupling: Optional[str]=None) -> dict:
    ...
    for tc,tab in context_tables.items():
        for ec in range(4):
            vals=[run_episode(model,start,"P1",tab,False,0,ec,disabled_coupling)["success"] for start in initial_states(model,"eval",ec)]
