...
class AgentServiceR13:
    """R13 composition: R12 semantic failover wired to A2A/MCP quiescence and effect evidence."""

    def __init__(self, r12: Any) -> None:
        self._r12 = r12

    def __getattr__(self, name: str) -> Any:
        return getattr(self._r12, name)
    ...
    def open(
        ...
        *,
        ...
        if mcp_tasks_caller is not None:
            quiescence_adapters["mcp"] = MCPTaskQuiescenceAdapter(mcp_tasks_caller)
        replay_adapter = (
            None
            if effect_ledger_reader is None
            else EffectLedgerReplaySafetyAdapter(effect_ledger_reader)
        )
        r12 = AgentServiceR12.open(
            db_path,
