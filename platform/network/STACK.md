...
R12 additionally proves local resilience semantics without adding components: sing-box address-family fallback works in both directions under deterministic single-family failure; dnsproxy parallel upstream mode survives one failed resolver, fails closed for uncached/expired names when all resolvers are unavailable, and recovers without process restart. A real WSL reboot during R12 also proved that the enabled `network-v2-r0.target` reconstructs dnsproxy, sing-box, blackbox_exporter and Prometheus automatically on the next WSL boot. Windows-host reboot plus WSL auto-launch remains a Workstation/bootstrap concern rather than a Network data-plane fidelity gate.
