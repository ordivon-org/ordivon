...
class ArtifactDonorExportTests(unittest.TestCase):
    ...
    def test_all_taxonomy_families_and_profiles_are_represented(self):
        ...
        self.assertEqual(c["normalizedProfiles"], 17)

    def test_all_shadow_bindings_remain_live_proven(self):
        self.assertEqual(self.donor["coverage"]["liveProvenShadowBindings"], 9)
    ...
    def test_all_isolated_shadow_validators_are_preserved_without_legacy_orchestrator(self):
        self.assertEqual(self.donor["coverage"]["isolatedValidatorImplementations"], 9)
    ...
    def test_candidate_problem_keys_are_explicitly_non_core(self):
        ...
        self.assertEqual(len({p["candidateProblemKey"] for p in self.donor["profiles"]}), 17)

    def test_observed_migration_gaps_are_preserved_not_invented_away(self):
        gaps={x["id"]:x for x in self.donor["knownGaps"]}
        self.assertEqual(gaps["object-contract-coverage"]["observation"], {"profilesWithExplicitObjectContract":8,"profilesWithoutExplicitObjectContract":9})
        self.assertEqual(gaps["result-boundary-coverage"]["observation"], {"profilesWithExplicitNonClaims":9,"profilesWithoutExplicitNonClaims":8})
        self.assertEqual(gaps["composition-sequencing"]["observation"], {"profilesWithEncodedSequence":0,"profilesWithoutEncodedSequence":17})
