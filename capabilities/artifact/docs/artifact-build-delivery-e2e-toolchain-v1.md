...
- Package assembly: `package-stage` separates VSA verification gates from output/provenance assembly gates, revalidates verify-receipt file facts before copying, copies authenticated Sigstore bundles under `attestations/`, produces relative-path package indexes, and composes the existing digest-bound release manifest. Local unsigned packages remain development-only (`releaseReady=false`); a signed package reaches `CRYPTOGRAPHICALLY_VERIFIED` / `releaseReady=true` only when every required VSA gate passes the configured signer/verifier trust policy.
...
- The Artifact Temporal worker unit is defined and verified in an isolated workspace, but main-source deployment remains intentionally unapplied until the candidate is integrated into `/root/projects/ordivon-artifact-v2`; detached-workspace verification is not production cutover.
