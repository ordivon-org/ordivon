...
`1f58012fa5a9f15d122e406b6b0c070df89431ff`
...
- 25 normalized profile-v2 entries;
- 17 live-proven shadow capability bindings;
- 17 isolated profile validator implementations across 10 standards-first families.
...
- **15/25** profiles have an explicit object contract; **10/25** do not.
- **17/25** profiles have explicit `nonClaims`; **8/25** do not.
- **0/25** profiles encode a universal execution sequence; all 25 remain `NOT_ENCODED_DO_NOT_INFER`.
...
Ten newer standards-first families now expose seventeen isolated profile-validator implementations: Audio has independent FLAC, RIFF/WAVE PCM, and Ogg/Vorbis profiles; Still Image has separate PNG and static SVG profiles; Design/2D has independent Tiled-native map and Workstation-managed Aseprite derivative validators. Software Release now keeps OCI image verification separate from bounded Linux ELF final-byte/readback verification. These are preserved as candidate validator capabilities:
