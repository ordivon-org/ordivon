...
| --- | --- | --- |
| `src/ordivon_skills/parser.py` | **TEMP-BRIDGE / REPLACE-UPSTREAM** | Needed only while the bridge ingests `SKILL.md`. Portable field semantics belong to Agent Skills. Prefer upstream/client-native validation when direct loading is available. |
| `src/ordivon_skills/config.py` standard `.agents/skills` discovery | **TEMP-BRIDGE / REPLACE-UPSTREAM** | Mirrors Agent Skills client guidance only for remote bridge discovery. Direct clients should discover these roots themselves. |
...
| `src/ordivon_skills/catalog.py` federated inventory | **TEMP-BRIDGE** | Exists only to expose standard assets to a remote consumer. Do not evolve into a universal registry. |
...
| `src/ordivon_skills/scanner.py` credential/private-key quarantine | **KEEP-LOCAL, MINIMAL** | Prevent accidental secret exfiltration through the remote bridge. Client-owned security policy; not portable metadata. |
...
| `scanner.py` cross-Skill dependency inference | **RETIRED / DO NOT REINTRODUCE** | Natural-language routing references remain advisory/security input only. The bridge may flag or sanitize cross-Skill routing directives, but it no longer converts prose into `declaredDependencies` / `requiredDependencies` or resolves a bridge-owned dependency graph. |
| `src/ordivon_skills/skillspector_adapter.py` | **RETIRED / EXTERNAL OWNER** | Zero current production consumers. SkillSpector remains an external Security/installation scanner with its native evidence surfaces; the Skills bridge no longer normalizes its output into an Ordivon-specific evidence schema. |
| `src/ordivon_skills/eligibility.py` client-specific eligibility adapters | **TEMP-BRIDGE** | Compatibility adapter only; never portable Agent Skills semantics. Retire per-client adapters as clients consume their own native metadata. |
| `src/ordivon_skills/model.py` bridge records/status objects | **TEMP-BRIDGE** | Internal implementation model only. Do not expose as a cross-Harness ontology. |
| `src/ordivon_skills/sep2640.py` standard Skills protocol projection | **KEEP PROTOCOL ADAPTER** | Implements the standard MCP Skills wire surface for the bridge. Keep only as long as this server is a supported consumer path. |
