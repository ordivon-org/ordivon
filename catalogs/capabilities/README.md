...
For web/browser/desktop interaction specifically, `../../.agents/skills/web-provider-routing/references/web-interaction-r1.json` is the machine-readable thin-first routing profile and `../../.agents/skills/web-provider-routing/scripts/route.py` overlays current local/caller-bound availability. Natural-language task interpretation remains Agent/caller-owned; the resolver never treats provider documentation as availability truth. The reusable Agent procedure lives at `.agents/skills/web-provider-routing/SKILL.md`.
...
- external standard/provider version knowledge: append-only records under `catalogs/authorities/observations/` where appropriate.
