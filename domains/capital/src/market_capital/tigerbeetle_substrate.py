...
class CapitalReservationBinding:
    ...
    def __post_init__(self) -> None:
        if not self.reservation_ref.strip():
            raise SemanticViolation("reservation_ref required")
        ...
        if self.amount <= 0:
            raise SemanticViolation("capital reservation amount must be positive")
    ...
    def __post_init__(self) -> None:
        ...
        if self.operation is TigerBeetleOperation.PENDING:
            if self.pending_id != 0:
                raise SemanticViolation("pending reservation transfer cannot reference pending_id")
        else:
            if self.pending_id == 0:
                raise SemanticViolation("pending resolution requires pending_id")
            if self.timeout_seconds != 0:
                raise SemanticViolation("pending resolution timeout must be zero")
        if self.operation is TigerBeetleOperation.VOID_PENDING_TRANSFER and self.amount != 0:
            raise SemanticViolation("void pending transfer amount must be zero")
...
def resolution_instruction(
    *,
    ...
    if not isinstance(disposition, EffectDisposition):
        raise SemanticViolation("disposition must be a Market Capital EffectDisposition")
    if not resolution_ref.strip():
        raise SemanticViolation("resolution_ref required")
    if resolution_ref == binding.reservation_ref:
        raise SemanticViolation("resolution identity must differ from reservation identity")
    if disposition is EffectDisposition.RETAIN:
        return None

    operation = (
        TigerBeetleOperation.VOID_PENDING_TRANSFER
        if disposition is EffectDisposition.RELEASE
        else TigerBeetleOperation.POST_PENDING_TRANSFER
    )
    amount = 0 if disposition is EffectDisposition.RELEASE else binding.amount
    return TigerBeetleTransferInstruction(
        operation=operation,
...
def stable_provider_id(namespace: str, *parts: str) -> int:
    if not namespace.strip() or not parts or any(not p.strip() for p in parts):
        raise SemanticViolation("provider identity binding requires non-empty namespace and parts")
...
def _require_u32(name: str, value: object) -> None:
    n = _require_int(name, value)
    if n < 0 or n >= 2**32:
        raise SemanticViolation(f"{name} must fit unsigned 32-bit range")
