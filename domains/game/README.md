...
- [`docs/GAME_FOUNDATIONS_CONTINUATION.md`](docs/GAME_FOUNDATIONS_CONTINUATION.md) is the context-switch handoff; after R29 the foundation-expansion programme is provisionally frozen; the next frontier is Pre-G0 Game Direction Search, with Foundations reopened only by a concrete counterexample.
...
- [`docs/GAME_PRE_G0_FORM_AGENT_ROLE_DECOUPLING.md`](docs/GAME_PRE_G0_FORM_AGENT_ROLE_DECOUPLING.md) is the current Pre-G0 search authority for separating what game to make from how Agents participate: broad traditional GameForm coverage first, Production Agents as production technology, and Runtime Agents only by cheaper-baseline admission.
- [`docs/GAME_PRE_G0_PLAYABLE_PROOF_PORTFOLIO.md`](docs/GAME_PRE_G0_PLAYABLE_PROOF_PORTFOLIO.md) freezes the ten-packet Pre-G0 player-facing proof portfolio and its C0/C1/C2 evidence gates without ranking GameForms or admitting G0.
