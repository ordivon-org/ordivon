...
- [`experiments/product-composition-r1/causal-lag-content-r4/README.md`](experiments/product-composition-r1/causal-lag-content-r4/README.md) applies the current Ordivon LEGO Question Compiler, C-K, compositional-contract and feedback-control lenses to **Core Breadth**. A thin content grammar compiles authored context-transition beats into the same R3 session kernel; the first seven-beat arc adds no player verbs and mechanically exposes 7 distinct policy signatures across the existing 3×3 transition space. Human value and product selection remain unobserved.
- [`experiments/product-composition-r1/causal-lag-pressure-r5/README.md`](experiments/product-composition-r1/causal-lag-pressure-r5/README.md) applies the newer LEGO Question Compiler / C-K / Feedback Control stack to **Pressure Topology**. Deterministic screening rejects target-only fake pacing, selects visible scan-cost pressure, and proves that its policy effect is strongly conditioned by the existing hidden stability regime. The ten-beat playable adds no player verbs and does not establish Human pacing quality or product value.
- [`experiments/product-composition-r1/causal-lag-horizon-r6/README.md`](experiments/product-composition-r1/causal-lag-horizon-r6/README.md) applies the updated LEGO Question Compiler + Lens Router to **Horizon Contract Planning**. Exact enumeration finds the current R5 composition space not yet mechanically saturated, while a bounded 1–3 beat dynamic-programming oracle finds 96 target-sensitive window states and 625 adjacent-target policy transitions. It remains an evaluator only: no browser carrier, no new player verb, and no Human/product claim.
