...
const atlas = JSON.parse(readFileSync(new URL("../assets/rescue-specialists.json", import.meta.url), "utf8"));
...
test("specialist Studio source and runtime derivative retain exact three-Actor mapping", () => {
  const source = new URL("../assets/rescue-specialists.aseprite", import.meta.url);
  const recipe = new URL("../assets/generate-rescue-specialists.lua", import.meta.url);
  const runtime = new URL("../web/assets/rescue-specialists.png", import.meta.url);
...
test("G4 audio keeps deterministic local source, mute state, and post-boundary cues", () => {
  const recipe = new URL("../assets/generate-g4-audio.sh", import.meta.url);
  assert.ok(existsSync(recipe));
  for (const cue of ["plan-ready", "commit", "aftermath"]) {
    const path = new URL(`../web/assets/audio/${cue}.ogg`, import.meta.url);
