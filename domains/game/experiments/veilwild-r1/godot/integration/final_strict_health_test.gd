...
func _readiness_witness(integration: Node) -> Dictionary:
    ...
    var ready := nav_ready and bool(observation.get("candidateReady", false)) and bool(observation.get("ownerWorldQualifierBound", false)) and String(observation.get("worldQualifierOwner", "")) == "A14/F14" and bool(animation.get("candidateReady", false)) and integration.load_failures.is_empty() and integration.integration_failures.is_empty()
    return {
        "ready": ready,
