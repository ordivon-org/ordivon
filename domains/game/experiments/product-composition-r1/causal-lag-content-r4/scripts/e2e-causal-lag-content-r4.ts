...
const pagePath='/experiments/product-composition-r1/causal-lag-content-r4/web/';
...
async function reset(options:any){
  await page.evaluate((o)=>((window as any).__CAUSAL_LAG_R4__.resetForAcceptance(o)),options);
}

try{
  await page.goto(base+pagePath,{waitUntil:'networkidle'});
  await page.getByRole('heading',{name:'Causal Lag · Breadth Arc'}).waitFor();
  await page.waitForFunction(()=>Boolean((window as any).__CAUSAL_LAG_R4__));

  assert.equal(await page.locator('[data-start]').isVisible(),true);
  assert.match((await page.locator('[data-contract]').textContent()??''),/TARGET · 560/i);
  ...
  assert.match((await page.locator('[data-role]').textContent()??''),/INTRODUCE/i);
  assert.match((await page.locator('[data-round]').textContent()??''),/ROUND 1 \/ 7/i);
  let snap=await page.evaluate(()=>((window as any).__CAUSAL_LAG_R4__.snapshot()));
  assert.equal(snap.current.currentContext.id,'cycle-1-baseline-output');
  assert.equal(snap.current.executionForecast.id,'cycle-1-baseline-output');

  await page.locator('[data-arch="route"]').click();
  await page.locator('[data-commit]').click();
  await page.locator('[data-next]').click();
  assert.match((await page.locator('[data-role]').textContent()??''),/PRACTICE/i);
  snap=await page.evaluate(()=>((window as any).__CAUSAL_LAG_R4__.snapshot()));
  assert.equal(snap.current.currentContext.id,'cycle-1-baseline-output');
  assert.equal(snap.current.executionForecast.id,'cycle-2-objective-shift');
  ...
  for(let i=0;i<7;i++){
    await page.locator('[data-arch="route"]').click();
    await page.locator('[data-commit]').click();
    if(i<6)await page.locator('[data-next]').click();
  }
  assert.match((await page.locator('[data-outcome]').textContent()??''),/CONTRACT SECURED/i);
  const success=await page.evaluate(()=>((window as any).__CAUSAL_LAG_R4__.snapshot()));
  assert.equal(success.sessionStatus,'SUCCESS');
  assert.equal(success.totalNet,700);
  assert.equal(success.history.length,7);
  ...
  const failure=await page.evaluate(()=>((window as any).__CAUSAL_LAG_R4__.snapshot()));
  assert.equal(failure.sessionStatus,'FAILURE');
  assert.equal(failure.totalNet,238);
  assert.equal(failure.contract.maxRecoverableTotal,538);

  await reset({
    regime:'VOLATILE',
    initialCause:'process',
    transitionScript:['route','source','process','route','source','process','route'],
  });
  const before=await page.evaluate(()=>((window as any).__CAUSAL_LAG_R4__.snapshot()));
  ...
  console.log(JSON.stringify({
    scenario:'causal-lag-content-r4',
    roles:['INTRODUCE','PRACTICE','VARY','COMBINE','STRESS','RECONTEXTUALIZE','CONCLUDE'],
