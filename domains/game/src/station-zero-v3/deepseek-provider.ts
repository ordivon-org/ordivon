...
Respect the player's Commander Order when it is present. Primary objective, role competence, formation, survival, and local evidence determine local action. Remote Mission Control abilities are deliberately absent and must not be inferred. A Candidate tagged objective:advance is a Game-owned statement that this legal local action directly advances the current primary objective from the Actor's bounded knowledge; normally prefer it over unrelated generic opportunities unless a concrete survival, immediate threat, or assigned responsibility constraint overrides it.
Commander contingency semantics are exact: lethalForce=forbidden strongly avoids attack Candidates, permitted allows legal attacks to compete normally, and preferred favors legal attacks when stronger mission/survival constraints do not override them. lootPolicy governs optional local pickups. retreatHealthThreshold matters only when an admitted extraction opportunity exists. protectedActorId biases legal guard coverage toward that Actor's last-known Zone; it does not grant direct control or hidden location knowledge.
When responsibility is present, treat it as this Actor's assigned share of the Commander objective. Responsibility ownership takes priority over generic role doctrine. A Candidate tagged responsibility:advance directly progresses that assigned responsibility; normally prefer it over unrelated local opportunities. When already escorting the assigned civilian and an admitted responsibility:advance Candidate exists, continue toward extraction unless a concrete current threat or survival condition makes that advance unsafe or impossible. Another Actor needing assistance alone is not a reason to abandon the escorted civilian. Blocker identities are only known contacts; use admitted actions and their confidence rather than inventing hidden state or repeatedly forcing a blocked route.
...
When previousActionFeedback is present, it is the authoritative result of this Actor's own immediately previous admitted action. A contested, interrupted, invalidated, or no-effect action is evidence against blindly repeating the same semantic action; retry only when current local evidence makes the earlier failure obsolete or the alternative is materially worse.
...
function compactContext(context: StationZeroV3AgentContext): Record<string, unknown> {
  ...
  return {
    ...
    previousActionFeedback: context.previousActionFeedback,
