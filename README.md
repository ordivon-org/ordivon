...
Use `ordivon_harness.api` as the supported application facade. The package root now exposes only `package_version`; it is not a second application facade. Advanced integrations import the explicit owner modules for Store, Continuity, Provider, Runtime, or recovery primitives; the duplicate `ordivon_harness.core` aggregation facade and historical `Standalone*` aliases are retired. Historical names remain documented in the changelog rather than kept as live compatibility surfaces.
