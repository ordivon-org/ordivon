...
- exact Contract and AgentTurnRequest projections in workbench/explain keep Run-admitted and turn-admitted action truth separate without maintaining an installed-capability registry;
- direct caller/owner-supplied `InteractionAffordance` facts (`AVAILABLE / BLOCKED / UNKNOWN`) that can only subtract from exact already-admitted Tools; Harness does not discover or rank owner standing;
- historical currentness-pressure experiments remain evidence that current standing can matter to Agent action choice; the generic lexical descriptor/candidate layer is retired, while the smaller direct InteractionAffordance path preserves caller-supplied standing plus admitted-Tool subtraction;
