...
def validate_model(model: Mapping, allow_selftest: bool = False) -> dict:
    ...
    if model["kind"] == "candidate":
        if model["candidateId"] not in {"R6C01","R6C02","R6C03"}:
            raise ValueError("invalid candidate id")
    elif not (allow_selftest and model["kind"] == "selftest" and model["candidateId"] in {"R6SELFTEST","R6SELFTEST01","R6SELFTEST02","R6SELFTEST03"}):
...
def run_episode(model: Mapping, start: str, policy: str, table=None, train=False, epoch=0, context=0, disabled_coupling=None) -> dict:
    ...
    for step in range(MAX_EPISODE_DECISIONS):
        ...
        if policy == "P0":
            ...
            if train and deterministic_float(policy,epoch,context,semantic_state_identity(model,start),step,key) < epsilon(epoch):
                idx = int(deterministic_float("pick",policy,epoch,context,semantic_state_identity(model,start),step,key) * len(legal)) % len(legal)
                action = legal[idx]
            else:
