...
| `ordivon-artifact-v2@e06b17a517c4` | clean; accepted forward source authority | artifact build/validation/delivery capability provider | **REGISTER_PROVIDER** | standards-first, external validators, OCI/OPA, explicit fail-closed release gates |
...
| `ordivon-host-v2@35b9defb1d2e` | clean external-first compatibility/product surface | compatibility/service candidate, not Core | **HOLD** | current architecture already questions Host as a durable category; do not import identity before consumer need is re-established |
