...
class FakeBoard:
    ...
    def post(
        self, *, client_message_id: str, author_label: str, message: str, topic: str
    ) -> BoardMessageRef:
        self.calls.append((client_message_id, author_label, message, topic))
        existing = self.messages.get(client_message_id)
        if existing is None:
            existing = BoardMessageRef(
                client_message_id=client_message_id,
                provider_sequence=len(self.messages) + 1,
            )
...
class AgentServiceGoalTests(unittest.TestCase):
    def _open(
        self, db: Path, board: FakeBoard | None = None
    ...
    def test_goal_links_tasks_without_making_board_or_task_store_second_owner(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                [task.id for task in service.goal_task_links.tasks_for_goal(goal.id)],
                [first.id, second.id],
            )
            self.assertIsNone(
                service.goal_task_links.goal_for_task(first.id).failure_reason
            )
    ...
    def test_readiness_is_derived_from_dependencies_not_persisted_as_task_state(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            service, _ = self._open(Path(tmp) / "service.db")
    ...
    def test_goal_reconciler_derives_running_success_and_failure_from_tasks(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            service, _ = self._open(Path(tmp) / "service.db")
            ...
            self.assertEqual(
                service.goal_reconciler.reconcile(goal.id).state, "PENDING"
            )
            with service._connection:
                service.tasks.set_state_in_transaction(a.id, "RUNNING")
            self.assertEqual(
                service.goal_reconciler.reconcile(goal.id).state, "RUNNING"
            )
            with service._connection:
                service.tasks.set_state_in_transaction(a.id, "SUCCEEDED")
                service.tasks.set_state_in_transaction(b.id, "SUCCEEDED")
            self.assertEqual(
                service.goal_reconciler.reconcile(goal.id).state, "SUCCEEDED"
            )
            ...
            with service._connection:
                service.tasks.set_state_in_transaction(
                    c.id, "FAILED", failure_reason="semantic failure"
                )
    ...
    def test_board_projection_uses_deterministic_message_identity_and_survives_response_loss(
        self,
    ) -> None:
        board = FakeBoard()
...
class AgentServiceBoardCatchupTests(unittest.TestCase):
    def test_project_pending_catches_up_every_goal_event_in_order(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(
                [r.service_event_id for r in receipts], [e.id for e in events]
            )


class AgentServiceGoalLegoDecompositionTests(unittest.TestCase):
    def test_goal_graph_exposes_separate_membership_dependency_readiness_and_guard_bricks(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            runtime = FakeRuntime()
