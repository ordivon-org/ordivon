...
def test_domain_governance_references_follow_current_domain_revisions():
    a = load("acceptance.json")
    assert (
        a["research"]["repositoryRevision"]
        == "18a8afae92ae2ef3b89d645bbcccf6db51db1b9d"
    )
    assert (
        a["finance"]["repositoryRevision"] == "14885beb2b566458123dd9f273a66884fb337017"
    )
...
def test_r4_moves_safe_binding_out_of_p0_without_claiming_resolution():
    ...
    assert {x["id"] for x in r4["p0Queue"]} == {
        "semantic-provenance",
        "decision-outcome-feedback",
    }
    fabric = next(
        x for x in r4["fabrics"] if x["name"] == "governance-rights-retention"
    )
