...
class AgentServiceMcpCanaryTests(unittest.TestCase):
    ...
    def test_server_exposes_exactly_three_read_only_tools(self):
        with tempfile.TemporaryDirectory() as tmp:
    ...
    def test_deployment_uses_standard_opentelemetry_zero_code_boundary(self):
        ...
        unit = Path("systemd/ordivon-agent-service-canary-mcp.service").read_text(
            encoding="utf-8"
        )

        self.assertIn("opentelemetry-distro[otlp]==0.65b0", dependencies)
        self.assertIn("opentelemetry-instrumentation-starlette==0.65b0", dependencies)
        self.assertIn("LoadCredential=agent-service-mcp.token:", unit)
        self.assertIn("--token-file %d/agent-service-mcp.token", unit)
        self.assertIn(".venv/bin/opentelemetry-instrument", unit)
        self.assertIn(
            "Environment=OTEL_SERVICE_NAME=ordivon-agent-service-canary", unit
        )
        ...
        self.assertNotIn(
            "opentelemetry",
            Path("agent_service/evidence.py").read_text(encoding="utf-8"),
        )
