...
DOGFOOD = ROOT / "evidence/acceptance/standard-native-enterprise-r2-dogfood-20260914.json"
ENTERPRISE = ROOT / "evidence/acceptance/enterprise-operating-model-r1-dogfood-20260914.json"
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        observation_files = [str(p) for authority_id in records for p in catalog.observation_paths(authority_id)]
        if record_files:
        ...
        temp_index.write_text(json.dumps(built, indent=2, ensure_ascii=False, sort_keys=True) + "\n", encoding="utf-8")
    ...
    forbidden_index_keys = {"officialSource", "access", "relations", "provenance", "role", "disposition", "rationale"}
    for entry in index["entries"]:
        leak = forbidden_index_keys.intersection(entry)
        if leak:
            fail(f"Level-0 discovery index leaked Level-1/task fields: {entry['id']} {sorted(leak)}")

    risk = [r for r in index["entries"] if r["id"] == "iso-31000-2018"]
    if len(risk) != 1:
        fail("ISO 31000 seed missing")
    found = [(catalog.score_entry(row, "risk management"), row["id"]) for row in index["entries"]]
    ...
    refresh = subprocess.check_output(
        [sys.executable, str(ROOT / "scripts/authority_catalog.py"), "refresh", "iso-31000-2018"],
        cwd=ROOT,
        text=True,
    )
    ...
    result = {
        ...
        "observationCount": sum(len(catalog.observation_paths(authority_id)) for authority_id in records),
