...
def _check_dag(stages: dict[str, dict[str, Any]]) -> None:
    ...
    for stage_id, stage in stages.items():
        ...
        if unknown:
            raise CircuitContractError(
                f"stage {stage_id} depends on unknown stages: {unknown}"
            )
...
def validate_manifest(value: dict[str, Any]) -> None:
    ...
    for stage_id, stage in stages.items():
        ...
        if missing_capabilities:
            raise CircuitContractError(
                f"stage {stage_id} references unknown capability bindings: "
    ...
    for edge in value["edges"]:
        ...
        if source["stageId"] not in target_stage["dependsOn"]:
            raise CircuitContractError(
                f"edge {edge['id']} requires {target['stageId']} to depend on "
                f"{source['stageId']}"
            )
    ...
    for gate_id, gate in gates.items():
        ...
        if producer["ownerId"] == consumer["ownerId"]:
            raise CircuitContractError(
                f"gate {gate_id} must cross an owner boundary in R1"
            )
...
def compile_manifest(value: dict[str, Any]) -> dict[str, Any]:
    ...
    required_gate_ids = sorted(
        gate["id"] for gate in value["gateRequirements"] if gate["required"]
    )
    compiled: dict[str, Any] = {
        ...
        "capabilityBindingIds": sorted(
            item["id"] for item in value["capabilityBindings"]
        ),
...
def validate_gate_result(
    ...
    if result["circuitId"] != manifest["circuitId"]:
        raise CircuitContractError(
            f"gate {result['gateId']} binds a different circuitId"
        )
    ...
    if gate is None:
        raise CircuitContractError(
            f"unknown composition gate result: {result['gateId']}"
        )
...
def evaluate_gate_results(
    ...
    required = {
        gate["id"]: gate for gate in manifest["gateRequirements"] if gate["required"]
    }
