...
PROFILE = json.loads(
    (SOURCE / "external-temporal-reference-profile.json").read_text(encoding="utf-8")
)
...
def duckdb_path_literal(path: Path) -> str:
    # DuckDB string literals escape an embedded single quote by doubling it.
...
def decode_revision(rev: dict) -> tuple[str, dict]:
    ...
    try:
        ...
        utf8_error = {
            ...
            "end": e.end,
            "reason": e.reason,
            "bytes": raw[e.start : e.end].hex(),
        }
    text = raw.decode(rev["encoding"])
    return text, {
        "declaredEncoding": rev["encoding"],
        "strictUtf8Valid": utf8_valid,
        "firstUtf8Error": utf8_error,
    }
...
def duckdb_schema(text: str, role: str) -> list[dict]:
    ...
    sql = f"DESCRIBE SELECT * FROM read_csv_auto('{duckdb_path_literal(p)}', header=true, sample_size=-1)"  # nosec B608
...
def schema_conformance_report(actual_schema: list[dict]) -> dict:
    fixtures = {}
    fixtures["actual_revision_pair"] = classify_schema_evolution(
    ...
    renamed = [
        {
            "name": (
                "organization_name" if x["name"] == "research_body" else x["name"]
            ),
            "type": x["type"],
        }
        for x in actual_schema
    ]
    fixtures["rename_without_mapping"] = classify_schema_evolution(
        actual_schema, renamed
    )
    ...
    for name, standing in expected.items():
        if fixtures[name]["standing"] != standing:
            raise RuntimeError(
                f"schema conformance fixture {name}: {fixtures[name]['standing']} != {standing}"
            )
    return {
        "schemaVersion": 1,
...
def bounded_replay_report(revisions: list[dict]) -> dict:
    processing_order = list(
        reversed(
            [{"commit": x["commit"], "commitTime": x["commitTime"]} for x in revisions]
        )
    )
...
def reference_identity_report(rows: list[list[str]]) -> dict:
    ...
    for id_col, meta in PROFILE["identity"]["referenceIdentifiers"].items():
        ...
        identifier_label_variants = {
            k: sorted(v) for k, v in id_to_labels.items() if len(v) > 1
        }
        report[id_col] = {
            "scheme": meta["scheme"],
            ...
            "standing": "PASS_IDENTIFIER_STABLE"
            if not ambiguous_labels
            else "REVIEW_AMBIGUOUS_LABEL",
        }
...
def main() -> None:
    ...
    write_json(
        BASE / "reference-identity-report.json",
        {
            ...
            "externalOwners": {
                "identifierResolution": PROFILE["externalOwners"][
    ...
    write_json(
        BASE / "activation-decision.json",
        {
            ...
            "iceberg": {
                "activate": False,
                "reason": "The observed real transition is representation-only; Git commits already provide immutable source snapshots, with no concurrent table writers or table-level schema evolution.",
            },
            "debezium": {
                "activate": False,
                "reason": "The source change log is Git history, not a database transaction log.",
            },
            "beam": {
                "activate": False,
                "reason": "The source is bounded Git revision history; no watermark/late-data runtime is required.",
            },
            "redpandaKafka": {
                "activate": False,
                "reason": "No unbounded fan-out/replay transport workload was demonstrated.",
            },
        },
    )
