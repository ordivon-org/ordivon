...
def materialize() -> dict:
    ...
    if target.exists():
        if digest(target)!=source_digest:
            raise RuntimeError("existing authority validator bytes differ from source digest")
    else:
        shutil.copyfile(SOURCE,target)
    payload={
        "schemaVersion":1,
        "kind":"ordivon.workstation.authority-validation-provider-materialization",
        "sourceSha256":"sha256:"+source_digest,
        "target":str(target),
        "targetSha256":"sha256:"+digest(target),
        "issuesAuthority":False,
