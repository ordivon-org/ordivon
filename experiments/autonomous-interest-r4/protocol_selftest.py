...
SCHEMA = ROOT / "domains/game/game-autonomous-interest-model-r4.schema.json"
PROTOCOL = ROOT / "domains/game/game-autonomous-interest-protocol-r4.json"
...
def test_tournament_no_external_digest():
    ...
    rows = [dict(base, candidateId="R4C01"), dict(base, candidateId="R4C02"), dict(base, candidateId="R4C03")]
...
def make_unique_observation_overflow_model():
    states = {}
    couplings = ["coupling-one", "coupling-two", "coupling-three"]
    for i in range(257):
        sid = f"s{i:03d}"
        if i == 256:
            states[sid] = {"observation": {"i": i}, "status": "WIN", "transitions": {}}
            continue
        nxt = f"s{i+1:03d}"
        transitions = {"a": {"next": nxt}, "b": {"next": nxt}}
        if i < 3:
            transitions["a"] = {"next": nxt, "couplingId": couplings[i], "withoutCouplingNext": sid}
        states[sid] = {"observation": {"i": i}, "status": "ONGOING", "transitions": transitions}
    ctx = {"trainInitialStates": [f"s{i:03d}" for i in range(4)], "evalInitialStates": [f"s{i:03d}" for i in range(4, 12)]}
    return {
        "schemaVersion": 1, "kind": "selftest", "candidateId": "R4SELFTEST", "humanSignalUsed": False,
        "actions": ["a", "b"], "causalCouplings": couplings,
        "contexts": {str(c): copy.deepcopy(ctx) for c in range(4)}, "states": states,
    }


def test_all_measured_hard_caps_enforced():
    overflow = make_unique_observation_overflow_model()
    try:
        er.validate_model(overflow, allow_selftest=True)
    except ValueError as exc:
        assert "uniqueObservationCount=257 > 256" in str(exc)
    else:
        raise AssertionError("257 unique observations bypassed measured hard cap")


def test_ablation_eligibility_is_evaluator_bound(model):
    winner = er.evaluate_model(model, allow_selftest=True)
    ablated = er.evaluate_model(model, allow_selftest=True, disabled_coupling="coupling-one")
    row = {"couplingId": "coupling-one", "winnerEvaluation": winner, "ablationEvaluation": ablated}
    assert sr.ablation_eligible(row) == (ablated["policySuccess"]["P3"] >= 0.50)
    fake = copy.deepcopy(row)
    fake["ablationEvaluation"]["policySuccess"]["P3"] = 0.49
    assert sr.ablation_eligible(fake) is False
    try:
        sr.choose_ablation([fake])
    except ValueError as exc:
        assert "no mechanically eligible ablation" in str(exc)
    else:
        raise AssertionError("P3=0.49 ablation was selectable")
    assert "playable" not in sr.choose_ablation.__code__.co_names


def test_renderer_conformance_is_frozen(model):
    report = rr.conformance_report(model)
    ablated_report = rr.conformance_report(model, "coupling-one")
    assert ablated_report["rendererId"] == rr.RENDERER_ID
    assert report["rendererId"] == rr.RENDERER_ID
    assert report["evalStarts"] == 32
    assert report["participantOrdinals"] == 12
    assert report["candidateSpecificAssetsAllowed"] is False
    assert report["externalNetworkAssetsAllowed"] is False
    hostile = copy.deepcopy(model)
    hostile["states"]["t0"]["observation"] = {"text": "</pre><script>bad()</script>"}
    rendered = rr.render_html(hostile, "t0")
    assert "<script>bad()</script>" not in rendered
    assert "&lt;script&gt;bad()&lt;/script&gt;" in rendered
    # Same model/state always yields identical Human-visible bytes.
    assert rr.render_html(model, "t0") == rr.render_html(copy.deepcopy(model), "t0")



def test_reference_digests_bound():
    ...
    for key, path in paths.items():
        ...
        assert digests[key] == actual, (key, digests[key], actual)
    assert protocol["humanRendererBoundary"]["rendererSha256"] == digests["renderer"]
...
def main():
    ...
    test_all_measured_hard_caps_enforced()
    test_ablation_eligibility_is_evaluator_bound(model)
    test_renderer_conformance_is_frozen(model)
    test_reference_digests_bound()
    test_protocol_zero_slots()
    print("R4_PROTOCOL_SELFTEST=PASS")
    print("R4_PRIOR_REGRESSIONS=PASS")
    print("R4_CANDIDATE_GENERATION_STARTED=false")
    print("R4_SEMANTIC_SLOTS_CONSUMED=0")
