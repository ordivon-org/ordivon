...
MANIFEST_KIND = "ordivon.game-autonomous-interest-r7-human-build-manifest"
...
def source_bindings() -> dict:
    return {
        ...
        "modelSchema": file_digest(ROOT / "domains/game/game-autonomous-interest-model-r7.schema.json"),
        "machineProtocol": file_digest(ROOT / "domains/game/game-autonomous-interest-protocol-r7.json"),
    }


def canonical_models(candidate_models: Sequence[Mapping]) -> list[Mapping]:
    if len(candidate_models)!=3:
        raise ValueError("exactly three candidate models required")
    ordered=sorted(candidate_models,key=lambda m:m["candidateId"])
    ids=[m["candidateId"] for m in ordered]
    if len(ids)!=len(set(ids)):
        raise ValueError("duplicate candidateId")
    return ordered


def presentation_seed(candidate_models: Sequence[Mapping]) -> str:
    ordered=canonical_models(candidate_models)
    digests=[er.semantic_model_fingerprint(m) for m in ordered]
    payload="ordivon-r7-presentation-blind-v1|"+"|".join(sorted(digests))
    return "sha256:"+hashlib.sha256(payload.encode("utf-8")).hexdigest()

def _body(candidate_models: Sequence[Mapping], *, allow_selftest: bool=False) -> dict:
    candidate_models=canonical_models(candidate_models)
    ...
    return {
        ...
        "presentationSeed":presentation_seed(candidate_models),
...
def make_manifest(candidate_models: Sequence[Mapping], *, allow_selftest: bool=False) -> dict:
    body=_body(canonical_models(candidate_models),allow_selftest=allow_selftest)
...
def validate_manifest(manifest: Mapping, candidate_models: Sequence[Mapping], *, allow_selftest: bool=False) -> dict:
    expected=make_manifest(canonical_models(candidate_models),allow_selftest=allow_selftest)
