...
- Clock Quality Gate: three independent NTP sources currently place the WSL/host clock about 2.65–2.68 s ahead; private/demo/live lanes fail closed at the 1000 ms gate. Windows-native inspection is itself blocked by the current VSOCK runtime-context failure.
...
- Crypto Stream Resilience R3: missing, stale, and time-diverged public feeds fail closed; dual-venue reconnect qualification is implemented but live fault injection remains blocked before workload by Surfpath control-plane serialization. Canonical evidence is exported through the existing node_exporter → Prometheus → Grafana stack.
