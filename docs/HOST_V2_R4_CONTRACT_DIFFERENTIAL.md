...
### Migration authority closure

The running Host service no longer executes a mutable latest-schema DDL bundle. `HostV2.initialize()` is now a read-only readiness fence: an empty/unmigrated database fails closed with an instruction to run `alembic upgrade head`, and only schema version 3 is admitted. The parallel runtime `schema.py` DDL authority has been deleted. Fresh verification proved the negative path on an empty database, then performed the real Alembic 0001→0002→0003 upgrade, reopened Host successfully, and passed 21/21 PostgreSQL/MCP tests plus Ruff and diff checks.
