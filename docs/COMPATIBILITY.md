...
The former `ordivon_harness.strategy_selection` module and its `Harness*StrategySelection*`, prior-attempt/Strategy-evidence aggregation, and selected-attempt helpers are retired. They had no current persisted objects or cross-repository runtime consumer beyond Harness' own acceptance/research surfaces. Higher-level orchestration now supplies an already-selected Mandate/Profile/Strategy directly to `compile_harness_attempt()`; historical evidence remains historical and is not rewritten.
