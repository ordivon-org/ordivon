...
- First Interface uses the smaller direct affordance carrier (`tool`, owner, standing, effect, requirements/respondsTo) and exact admitted-Tool subtraction; there is no parallel capability candidate/descriptor carrier.
...
- Observation-only source recovery now composes its exact Runtime-backed Tool catalog/grant directly in `observation_tool_surface`; the former generic `HarnessAgentRunToolSurface` wrapper is retired rather than becoming a plugin layer.
...
- Source/tool discovery and ranking, large-WorkingSet inspection strategy, reusable-source repository ownership, procedure evaluation and canonical promotion remain Agent/application/Host/domain or mature-substrate questions. Harness provides only advanced exact reference-to-seed admission for already-selected cross-Run sources.
