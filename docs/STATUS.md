...
- structured completion binds caller-defined result shape into the Run Contract. Provider results use a versioned one-MiB-bounded terminal carrier instead of the 8,000-byte human/control summary; legacy summary-encoded results remain readable. Current local structural validation is selected by the standard JSON Schema Draft 2020-12 `$schema` URI. Historical policy-absent v1 remains provider-constrained but not locally schema-verified, and the old `local-json-schema-draft-2020-12-profile-v1` token is retained only to read/validate already-persisted Contracts; semantic/evidence admission remains caller/domain authority.
