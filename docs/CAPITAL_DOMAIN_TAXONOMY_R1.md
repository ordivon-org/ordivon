...
This source-owner migration does **not** silently rewrite protocol/evidence identities. Existing `ordivon.capital.market.*` JSON kinds are versioned contract identifiers embedded in schemas, configs and frozen evidence. Renaming them requires an explicit versioned contract migration. Likewise the deployed `ordivon_market_capital_*` Prometheus family and historical `Market Capital` evidence/fixture names remain registered compatibility/provenance identities.
