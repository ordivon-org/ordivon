...
def network_v2_ws_proxies() -> tuple[str, str]:
    ...
    if not okx or not binance:
        raise RuntimeError("Ordivon Capital Markets-domain public streaming requires exact Network v2 OKX/Binance Spot WS proxy bindings")
    return okx, binance
...
async def capture_streaming(rounds: int = 3, warmup_rounds: int = 1, deadline_seconds: float = 25.0) -> dict[str, Any]:
    ...
    return {
        "schemaVersion": 2,
        "kind": "ordivon.capital.markets.crypto-public-streaming-r2",
        "standing": standing,
