...
from campaign_birth import CampaignLaunchSpec, compile_campaign
...
def _birth_for(spec: CampaignLaunchSpec, agent_id: str):
...
async def run(a) -> None:
    ...
    if a.operation == "campaign-birth":
        if a.agent_id or a.prompt_file or a.turn_request_id or a.resume_id:
            raise ValueError("campaign-birth does not accept agent/turn/resume arguments")
        ...
        results = await asyncio.gather(
            *(
                _admit_workflow(
                    client,
                    AGENT_BIRTH_WORKFLOW,
                    AgentBirthInput(spec_path=str(a.spec.resolve()), agent_id=birth.agent_id),
                    workflow_id=birth.effect_id,
        ...
        admissions = [
            {
                ...
                "workflowType": AGENT_BIRTH_WORKFLOW,
                "disposition": result["disposition"],
                **({"runId": result["runId"]} if "runId" in result else {}),
            }
            for birth, result in zip(births, results, strict=True)
        ]
        out = {
    ...
    elif a.operation == "agent-birth":
        if not a.agent_id or a.prompt_file or a.turn_request_id or a.resume_id:
            raise ValueError("agent-birth requires exactly --agent-id")
        birth = _birth_for(spec, a.agent_id)
        workflow_id = birth.effect_id
        admission = await _admit_workflow(
            client,
            AGENT_BIRTH_WORKFLOW,
            AgentBirthInput(spec_path=str(a.spec.resolve()), agent_id=a.agent_id),
        ...
        birth = _birth_for(spec, a.agent_id)
        # Reconciliation is a repeatable observation of one existing Birth effect, not a new effect.
