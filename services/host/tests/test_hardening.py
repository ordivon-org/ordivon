...
from ordivon_host_v2.extensions import ExtensionStore
...
def stores() -> tuple[HostV2, BoardStore, NewsStore, ExtensionStore]:
    assert DSN is not None
    host = HostV2(DSN)
    host.initialize()
    return host, BoardStore(DSN), NewsStore(DSN), ExtensionStore(DSN)
...
def test_extension_history_is_immutable_per_task_namespace_revision() -> None:
    host, _, _, extension = stores()
    task_id = f"task:v2:extension:{uuid4().hex}"
    host.adopt(
        task_id=task_id,
        checkpoint=CheckpointInput(payload={"frontier": "one"}),
        client_request_id=f"adopt:{task_id}",
    )
    first = extension.put(
        task_id=task_id, namespace="world", expected_task_revision=1, payload={"value": 1}
    )
    replay = extension.put(
        task_id=task_id, namespace="world", expected_task_revision=1, payload={"value": 1}
    )
    assert replay["payloadDigest"] == first["payloadDigest"]
    ...
    historical = extension.get(task_id=task_id, namespace="world", at_revision=1)
    assert historical is not None
    assert historical["payload"] == {"value": 1}


def test_reserved_task_route_anchor_rejects_squatting_and_attention_routes_reply() -> None:
    host, board, _, _ = stores()
    ...
    delta = host.attention_delta(after_sequence=int(anchor["message"]["sequence"]), limit=10)
