...
- owns one systemd unit and cgroup process tree per Linux Attempt; a native Windows Runtime node owns `windows_native` Attempts through Windows SCM and Job Objects, without a WSL/systemd carrier;
...
Portable unit and protocol checks run on ordinary Linux CI. Real Linux process-supervision acceptance requires the local systemd/cgroup environment. `windows_native` is owned only by a native Windows Runtime node: the node runs as a Windows service, uses the repository-owned Job Object launcher, and does not depend on WSL, `WSL_INTEROP`, Linux systemd, Linux cgroups, or `\\wsl.localhost` path projection. New Windows Jobs execute under a verified limited native token by default and freeze a bounded Windows-native baseline environment at admission. `workspace.execBound` supports limited-only Windows immutable-input presentation; explicit elevated input-bound admission is rejected, while ordinary effect-opaque Windows execution may explicitly select already-present elevated provider authority. Every active Windows Attempt owns an Attempt-scoped `SystemRequired` Power Request, and native Runtime reconstruction/reconciliation uses durable launcher/Job Object identity rather than a Linux supervisor identity. Linux and Windows Runtime nodes therefore remain independently available and advertise only their native execution target.
