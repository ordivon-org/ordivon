...
| internal ordinary-operation circuit compilation | R08 ordinary-family pilot qualified on `397dee5ef0`: the existing `SubmitRequest` is compiled only after R07 authority binding and owner validations; exact submit-shape parity plus fail-closed authority/request/plan drift tests pass; input-bound and credential-bound families are not yet migrated |
