...
| internal operation-circuit compilation | R08 ordinary execution is integrated on `397dee5ef0`, and reduced immutable-input execution is current-main qualified on `40c414a390`: both compile the existing `SubmitRequest` only after R07 authority binding and the family-specific owner validations; reduced input keeps the existing materialization/preallocation path and additionally binds the realized read-only effective input set back to the AuthorityContract. Trusted immutable-input and credential-bound trusted execution remain unmigrated |
