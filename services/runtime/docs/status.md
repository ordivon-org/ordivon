...
macOS, non-systemd Linux, hostile multi-tenant execution, controlled egress, and disposable remote sandboxes are not currently supported Runtime authority profiles. `executionTarget=windows_native` is provided only by a native Windows Runtime node. Linux Runtime nodes advertise `local_linux` only; Windows Runtime nodes advertise `windows_native` only. Cross-node capability discovery/routing belongs above Runtime.
...
| internal execution AuthorityContract compilation | implemented as R07 on `163db32304`: after exact existing-Job replay lookup, the current public execution families compile to one internal fail-closed authority contract without changing public MCP Tool schemas, inventing Security/provider/domain permission, or widening target/profile/input/credential/Windows authority |
| internal operation-circuit compilation | R08 is complete for all four current effect-opaque execution families: ordinary `397dee5ef0`, reduced immutable-input `40c414a390`, trusted immutable-input `645d50cd51`, and credential-bound trusted `c52737fd7b` all compile through the internal `AuthorityContract + OperationCircuitCompiler` seam into the existing `SubmitRequest`. Immutable-input and credential materialization stay with their existing owners; exact replay, request identity, Registry v6, provider authority, and semantic-completion ownership are unchanged. Runtime self-release remains a separate reconciliable effect |
| internal execution-provider SPI | R09 qualified on `fd3f166024`: a private static `ExecutionProviderSpi` freezes `capabilities / validate / realize / observe / cancel / reconcile` obligations plus non-weakenable R1 guarantees for committed provider identity, one physical Attempt owner, terminal observation, process-tree cancellation, crash/restart reconciliation, and scoped evidence. Linux and Windows retain distinct owner/observation/evidence types; no dynamic provider registry, marketplace, cross-node router, public Tool change, or persisted provider schema was added. Full Runtime owner verification PASS: `job-01a0d34d-18e1-7140-b577-c34c0c660b4a` |
...
| `windows_native` execution target and Windows Job Object ownership | native Windows-node backend; real Runtime Job/Attempt success, frozen Windows baseline environment, limited/elevated ordinary execution, limited-only immutable-input presentation, replay, timeout/cancel, evidence, SCM recovery, WSL-independence, and descendant cleanup verified on the native Windows candidate |
...
| structured Workspace Patch effect | retired; the dedicated Patch control plane duplicated durable effect lifecycle already owned by Runtime Jobs. Text mutation remains `workspace.mutate`; response-loss-safe mutation uses `workspace.exec` with the chosen mature tool under the existing Job/Attempt ledger |
...
- arbitrary command execution remains effect-opaque. Runtime self-release is the only currently graduated structured external effect contract; retiring Workspace Patch deliberately reduced duplicate effect machinery rather than generalizing it.
...
- Windows execution is native-node only. SCM owns daemon lifecycle and the Windows launcher/Job Object owns Attempt process-tree lifecycle; WSL/systemd carrier recovery is historical evidence, not a current execution path. Native reconstruction reattaches the same durable Attempt from Windows owner evidence, and exact replay never redispatches an ambiguous external effect.
