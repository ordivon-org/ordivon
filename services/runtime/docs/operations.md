...
For a full release, the deployment invocation also defines the future structured-release authority: ORDIVON_RELEASE_SOURCE_REPO is the canonicalized --source-repo owner root and ORDIVON_RELEASE_REQUIRED_REF is --require-ref. The deployer receipts both their previous and candidate values. Together with the two Runtime timeout-policy values, these four owned environment keys are compare-and-swapped through one atomic environment-file replacement; unrelated installation settings and credentials remain untouched. Binary-subset maintenance owns neither timeout policy nor structured-release authority.
...
The same full-release commit cut owns the structured-release authority transition. A standalone-to-monorepo migration therefore does not edit the service environment as a separate side effect: the old source/ref authority and the candidate source/ref authority are recorded in the receipt and transition in the same atomic environment-file replace as the timeout policy before artifact commit. Authority or policy drift between pre-drain observation and the commit cut fails closed.
...
Because the authority transition occurs only at the fenced commit cut, a drain timeout cannot partially migrate ORDIVON_RELEASE_SOURCE_REPO or ORDIVON_RELEASE_REQUIRED_REF.
...
For receipts that carry a full-release authority transition, rollback restores ORDIVON_RELEASE_SOURCE_REPO and ORDIVON_RELEASE_REQUIRED_REF together with ORDIVON_DEFAULT_RUNTIME_MS and ORDIVON_MAX_RUNTIME_MS through one atomic environment update. Older schema-v2 receipts without releaseAuthority remain readable and retain their previous rollback semantics.
