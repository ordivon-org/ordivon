...
- the configured release source is the canonical Runtime owner directory; in a standalone repository its owner prefix is empty, while in the modular monorepo it is `services/runtime`;
- repository revisions are resolved to the latest commit at or before that revision that actually touched the Runtime owner, so sibling-owner commits do not create a new Runtime release identity;
- for a nested Runtime owner, the required repository ref authorizes the candidate only when the candidate owner Commit is an ancestor of the required-ref Commit and both revisions resolve to the same Runtime owner tree; the plan records both the repository-level required-ref Commit and the owner-scoped Runtime Commit;
- a full release treats `ORDIVON_RELEASE_SOURCE_REPO` and `ORDIVON_RELEASE_REQUIRED_REF` as receipted deployment state: they are compare-and-swapped atomically with Runtime-owned timeout policy after ingress is stopped, and rollback restores their previous values together with the previous artifact set;
