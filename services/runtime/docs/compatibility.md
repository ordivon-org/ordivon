...
| retired Workspace Patch receipt table | none in the current binary; v6 explicitly drops the former side table after the dedicated Patch control plane was retired | rollback to a binary that still requires the former table is outside the current schema-compatibility window | v6 is the retirement boundary; current Runtime uses Job/Attempt durability for response-loss-safe execution rather than recreating the Patch table |
