...
Independent Provider Call writes now use schema v4, and the exact schema-v3 decoder remains only for retained historical Provider Call objects. The obsolete caller-neutral Provider Call v2 typed codec is no longer a supported reader or writer; current estate evidence shows its remaining bytes only as superseded historical references in completed experiment roots, which remain inspectable through the generic Journal/CAS read surfaces. Dispatch Fence v2 remains current where Runtime effect fencing requires its independent Run revision and `ordivon.harness` authority projection.
