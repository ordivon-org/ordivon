...
The current Harness base runtime dependency graph is intentionally small and Host-free: the package depends on pinned `httpx==0.28.1` for cancellable DeepSeek HTTP/TLS transport and `jsonschema` for opt-in local structured-result conformance verification. Standard Agent Plugin MCP consumption is an explicit `mcp` standards-adapter extra pinned to the official `mcp==2.2.0` Python SDK; importing the base Harness API does not require that extra. The exact third-party transitive graph is pinned by `uv.lock` and mirrored in `requirements-audit.txt`. There is no Host dependency, optional Host extra, or Host development group.
