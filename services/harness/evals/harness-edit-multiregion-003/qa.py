...
FIXTURE = REPO / "fixtures/harness-edit-multiregion-v1"
ORACLE = ROOT / "oracle/pipeline.py"
KNOWN_INVALID = ROOT / "known-invalid/collateral.py"
HIDDEN_VERIFIER = ROOT / "verifier/test_outcome.py"
VISIBLE_FILES = ("SPEC.md", "pipeline.py", "test_pipeline.py")
PROTECTED_FILES = ("SPEC.md", "test_pipeline.py")
...
def validate_task() -> dict[str, Any]:
    ...
    if task.get("taskId") != "HARNESS-EDIT-MULTIREGION-003" or task.get("taskVersion") != 1:
        raise ValueError("Task version differs")
    if task["initialState"]["fixturePath"] != "fixtures/harness-edit-multiregion-v1":
...
def run_candidate(label: str, source: Path | None) -> dict[str, Any]:
    with tempfile.TemporaryDirectory() as directory:
        ...
        if source is not None:
            shutil.copyfile(source, workspace / "pipeline.py")
        env = {
            **os.environ,
        ...
        visible = _run(
            ["/usr/bin/python3", "-m", "unittest", "-q", "test_pipeline.py"],
            cwd=workspace,
            env=env,
        )
        ...
        unexpected = [
            path for path in changed if path != "pipeline.py" and not path.startswith("artifacts/")
        ]
        if unexpected:
...
def run_qa() -> dict[str, Any]:
    ...
    for index in range(1, task["reproducibility"]["cleanRebuildTrials"] + 1):
        cases = {
            ...
            "timeout-only": run_candidate("timeout-only", ROOT / "known-invalid/timeout-only.py"),
            "retry-only": run_candidate("retry-only", ROOT / "known-invalid/retry-only.py"),
            "collateral": run_candidate("collateral", ROOT / "known-invalid/collateral.py"),
        }
        expected = {
